...
Failed checks automatically create HITL queue items in `review/meetings/hitl-queue.jsonl`:
...
Optional YAML config at `config/quality-harness.yaml`:
...
gate = QualityGate(config_path=Path("config/quality-harness.yaml"))
